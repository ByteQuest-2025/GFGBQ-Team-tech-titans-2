...
function LinkVerificationCard({ link }) {
  ...
  return (
    <motion.div 
      className={`bg-white rounded-lg border-l-4 ${getStatusColor(link.status)} shadow-sm p-4 mb-3`}
          ...
          {link.details.archived && link.details.archive_url && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <motion.a 
                  href={link.details.archive_url} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="text-sm text-blue-700 hover:text-blue-900 inline-flex items-center gap-1 break-all"
                  whileHover={{ x: 5 }}
                >
